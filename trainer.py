...
def train(opts, image_data_loader, generator, discriminator, extractor, generator_optim, discriminator_optim, is_cuda):
    ...
    csv_file_path = os.path.join(opts.save_dir, 'losses.csv')  # 定义CSV文件路径
    csv_headers_written = False  # 标记是否已经写入CSV表头
    for index in pbar:
            ...
            if i % 100 == 0:
                if not csv_headers_written:
                    with open(csv_file_path, 'w', newline='') as csvfile:
                        ...
                        writer_csv.writerow(['g_loss_joint', 'd_loss_joint'])
                        csv_headers_written = True

                with open(csv_file_path, 'a', newline='') as csvfile:
                    writer_csv = csv.writer(csvfile)
                    writer_csv.writerow([pbar_g_loss_joint, pbar_d_loss_joint])
